...
if __name__ == "__main__":
    ...
    u_lb = - u_ub # 为什么下界和上届一样？？他妈的那么大一个负号
    ...
    H_sl = 20 # 测试loop的步长，时间间隔0.2s，总共4s

    # Generate self loop prediction input sequence # sl指self loop
    U1_sl = 0.5 * np.sin(np.linspace(0, 2 * np.pi, H_sl))
    U2_sl = - U1_sl # 两个正弦的输入，u2是u1的取反
    ...
    animate(X_ref[:-H], [X_mpc], ['MPC'], fps=1 / tau,save_ani=True)
